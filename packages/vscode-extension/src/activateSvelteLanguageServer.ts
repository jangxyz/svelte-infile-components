...
export function activateSvelteLanguageServer(context: ExtensionContext) {
  ...
  //_addMigrateToSvelte5Command(getLS, context);
  //_addOpenLinkCommand(context);
...
function _addMigrateToSvelte5Command(
  getLS: () => LanguageClient,
  context: ExtensionContext,
) {
  context.subscriptions.push(
...
function _addOpenLinkCommand(context: ExtensionContext) {
  context.subscriptions.push(
