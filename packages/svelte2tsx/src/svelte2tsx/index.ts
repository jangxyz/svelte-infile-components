...
} from "./processModuleScriptTag";
import path from "path";
import { parse, VERSION } from "svelte/compiler";
import { preRecursiveProcess } from "./infileComponents";
import { ffilename } from "./infileComponents_utils";
...
export function svelte2tsx(
    ...
    console.log(
        "🚀 ~ file: svelte2tsx/index.ts:52 ~ svelte2tsx:",
            ...
            const hoist_to_module =
                ...
                (globals.size === 0 ||
                    [...globals.keys()].every((id) =>
                        exportedNames.hoistableInterfaces.isAllowedReference(
                            id,
                        ),
                    ));
    ...
    addComponentExport({
        str,
        canHaveAnyProp:
            !exportedNames.uses$$Props && (uses$$props || uses$$restProps),
        events,
        isTsFile,
        exportedNames,
        usesAccessors,
