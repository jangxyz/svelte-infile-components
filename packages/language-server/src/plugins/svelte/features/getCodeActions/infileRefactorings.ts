...
export async function executeExtractInfileComponentCommand(
    ...
    const isValidSelectionRange =
        ...
        checkIsSveltePart(range, svelteDoc);
    if (!isValidSelectionRange) {
        console.log("🚀 ~ file: infileRefactorings.ts:89:", "Invalid selection range", {
    ...
    function insert_ImportInfileComponent(module: string): TextEdit {
        ...
        const importText = `\n  import ${componentName} from 'infile:${module}.svelte';\n`;
...
export function checkIsTouchingWhitespace(
    ...
    const validStart =
        offsetStart === 0 || /[\s\W]/.test(text[offsetStart - 1]);
    const validEnd =
...
export async function executeMoveInfileComponentToNewFileCommand(
    ...
    const { range } = refactorArgs;
    console.log(
        "🚀 ~ file: infileRefactorings.ts:219 ~ executeMoveInfileComponentToNewFileCommand:",
        svelteDoc,
        { refactorArgs },
    );

    //const isValidSelectionRange = checkIsTouchingWhitespace(range, svelteDoc) && checkIsSveltePart(range, svelteDoc);
    //if (!isValidSelectionRange) {
    //    //console.log("🚀 ~ file: infileRefactorings.ts:228 ~ isValidSelectionRange:", isValidSelectionRange, { isTouchingWhitespace: checkIsTouchingWhitespace(range, svelteDoc), isSveltePart: checkIsSveltePart(range, svelteDoc), range });
    //    return "Invalid selection range";
    //}
    ...
    if (!infileComponentInfo) {
        return "Not inside an infile component";
    }

    console.log(
        "🚀 ~ file: jangxyz_infileRefactorings.ts:247 ~ infileComponentInfo:",
        infileComponentInfo, { range }
    );
    ...
    //console.log( "🚀 ~ file: infileRefactorings.ts:265:", { newFileUri, componentName, filePath, }, inspect( [ clear_infileComponent(infileComponentInfo), replace_ImportInfileComponent(componentName, filePath), TextEdit.insert(Position.create(0, 0), newFileText), ], { depth: 4 },),);
    ...
    function clear_infileComponent(infileComponentInfo: InfileComponentInfo) {
        const { start, end } = infileComponentInfo.offsetRange;
        ...
        //console.log('🚀 ~ file: jangxyz_infileRefactorings.ts:302 ~ markupText:', markupText);
    ...
    function replace_ImportInfileComponent(
        ...
        const matchAllData = text.matchAll(
            /(import\b.*?['"])infile:(.*?)(['"])/g,
        );
        return [...matchAllData]
            .map((md, _index) => {
                const [_, preString, moduleName, quot] = md;
                const bareCompName = moduleName.replace(/[.]svelte$/, "");
                ...
                //console.log("🚀 ~ file: infileRefactorings.ts:352 ~ newText:", _index, md, { startPos, endPos, range, newText, })
    ...
    function dedent(text: string) {
        ...
        const indentSizes = lines
            ...
            .filter(
                (size): size is number => typeof size === "number" && size > 0,
            );
...
function updateRelativeImports(
    ...
    while (match) {
        ...
        const newImportPath = updateRelativeImport(
            oldPath,
            newPath,
            importPath,
        );
...
export function getCurrentInfileComponent(
    ...
    function parseInfileComponent(text: string) {
        const frontAllMatchData = [
            ...text
                .slice(0, startOffset)
                .matchAll(/\n---\n\s*(<template[^>]*>)/sg)
        ];
        if (frontAllMatchData.length === 0) return null;
        const frontMatchData = frontAllMatchData.at(-1)!;

        const rearMatchData = text
            .slice(startOffset)
            .match(/(.*?)<\/template>/s);
        if (!rearMatchData) return null;

        const infileComponentOffsetRange = {
            start: frontMatchData.index!,
            end: startOffset + rearMatchData[0].length,
        };
        const { start, end } = infileComponentOffsetRange;
        const infileComponentRange = Range.create(
            positionAt(start, text),
            positionAt(end, text),
        );
        ...
        const infileComponentText = text.slice(
            infileComponentOffsetRange.start,
            infileComponentOffsetRange.end,
        );
        const frontText = frontMatchData.input.slice(frontMatchData.index + frontMatchData[0].length);
        const innerText = frontText + rearMatchData[1];

        //const { start, end } = infileComponentInfo.range;
        //const targetRange = Range.create( positionAt(start, text), positionAt(end, text),);
        //console.log("🚀 ~ file: infileRefactorings.ts:504 ~ parseInfileComponent :", { name: infileComponentName, offsetRange: infileComponentOffsetRange, range: infileComponentRange, text: infileComponentText, innerText, }, { range, startOffset, frontMatchData, rearMatchData, infileComponentOffsetRange, infileComponentRange, fileNameMatchData, infileComponentName, infileComponentText, })

        return {
            name: infileComponentName,
            offsetRange: infileComponentOffsetRange,
