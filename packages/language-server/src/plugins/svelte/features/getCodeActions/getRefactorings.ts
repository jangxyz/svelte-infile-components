...
} from './infileRefactorings';
import {
  executeExtractInfileComponentCommand,
  executeMoveInfileComponentToNewFileCommand,
  extractInfileComponentCommand,
  moveInfileComponentToNewFile,
} from './infileRefactorings';
...
export async function executeRefactoringCommand(
  ...
  //console.log('🚀 ~ file: getRefactorings.ts:41 ~ command:', JSON.stringify(command), { svelteDoc, args });
  if (command === extractComponentCommand && args) {
    return executeExtractComponentCommand(svelteDoc, args[1]);
  } else if (command === extractInfileComponentCommand && args) {
    return executeExtractInfileComponentCommand(svelteDoc, args[1]);
  } else if (command === moveInfileComponentToNewFile && args) {
    const result = await executeMoveInfileComponentToNewFileCommand(
      svelteDoc,
      args[1],
    );
    console.log(
      '🚀 ~ file: getRefactorings.ts:48 ~ executeRefactoringCommand:',
