...
export async function getDiagnostics(
  ...
  console.log(
    '🚀 ~ file: svelte/getDiagnostics.ts:33 ~ { document, svelteDoc, settings, cancellationToken }:',
    { document, svelteDoc, settings, cancellationToken },
  );
  const config = await svelteDoc.config;
  console.log('🚀 ~ file: svelte/getDiagnostics.ts:37 ~ config:', config);
  ...
  try {
    console.log('🚀 ~ file: svelte/getDiagnostics.ts:47 diving in...');
...
async function tryGetDiagnostics(
  ...
  console.log(
    '🚀 ~ file: svelte/getDiagnostics.ts:70 ~ tryGetDiagnostics ~ { document, svelteDoc, settings, cancellationToken, transpiled }:',
    { document, svelteDoc, settings, cancellationToken, transpiled },
  );
  if (cancellationToken?.isCancellationRequested) {
    return [];
  }
  ...
  try {
    ...
    console.log('🚀 ~ file: svelte/getDiagnostics.ts:79 ~ res:', res);
    if (cancellationToken?.isCancellationRequested) {
      return [];
    }
    ...
    console.log('🚀 ~ file: svelte/getDiagnostics.ts:126 ~ result:', result);
    return result;
  } catch (err) {
...
function getStyleErrorDiagnostics(
  ...
  function getStyleErrorMessage() {
    if (isSveltePreprocessCannotFindModulesError(error)) {
      const hint = error.message.includes('node-sass')
        ? scssNodeRuntimeHint
        : '';
