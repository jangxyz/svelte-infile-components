    ...
    SelectionRangeProvider
{
  ...
  async getDiagnostics(
    ...
    result.then((value) => {
      console.log(
        '🚀 ~ file: SveltePlugin.ts:114 ~ getDiagnostics ~ then:',
        value.length,
        value,
        { document },
      );
  ...
  async formatDocument(
    ...
    const { prettier, config, isFallback, resolvedPlugins } =
      await importFittingPrettier();
  ...
  async getCompletions(
    ...
    const svelteDoc = await this.getSvelteDoc(document);
    console.log('🚀 ~ file: SveltePlugin.ts:319 ~ svelteDoc:', svelteDoc);
