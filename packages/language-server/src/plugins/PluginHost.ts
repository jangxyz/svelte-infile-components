...
export class PluginHost implements LSProvider, OnWatchFileChanges {
  ...
  async getDiagnostics(
    ...
    console.log(
      '\n' +
        '🚀 ~ file: PluginHost.ts:86 ~ PluginHost ~ getDiagnostics ~ textDocument:',
      textDocument,
    );
  ...
  async getCompletions(
    ...
    console.log(
      '🚀 ~ file: PluginHost.ts:148 ~ PluginHost ~ getCompletions:',
      inspect(completions),
      {
        textDocument,
        position,
  ...
  async getDefinitions(
    ...
    console.log(
      '🚀 ~ file: PluginHost.ts:336 ~ PluginHost ~ textDocument:',
      textDocument,
      { position },
    );
    const document = this.getDocument(textDocument.uri);
    ...
    if (this.pluginHostConfig.definitionLinkSupport) {
      ...
      return definitions.map(
        (def) =>
          <Location>{ range: def.targetSelectionRange, uri: def.targetUri },
      );
    }
  }
  ...
  private async execute<T>(
    ...
    const plugins = this.plugins.filter(
      (plugin) => typeof plugin[name] === 'function',
    );
    if (
      ![
        'getInlayHints',
      ...
      console.log(
        '🚀 ~ file: PluginHost.ts:723 ~ PluginHost ~ execute:',
        JSON.stringify(name),
        plugins,
        { args, mode, priority },
      );
  ...
  private async executePlugins(
    ...
    switch (mode) {
      ...
      case ExecuteMode.Collect:
        return Promise.all(
          plugins.map((plugin) =>
            this.tryExecutePlugin(plugin, name, args, []),
          ),
        );
      case ExecuteMode.None:
        await Promise.all(
          plugins.map((plugin) =>
            this.tryExecutePlugin(plugin, name, args, null),
          ),
        );
        return;
    }
