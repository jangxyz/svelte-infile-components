    ...
    UpdateTsOrJsFile
{
  ...
  constructor(
    ...
    console.log(
      '🚀 ~ file: TypeScriptPlugin.ts:171 ~ { configManager, lsAndTsDocResolver, workspaceUris, documentManager }:',
      { configManager, lsAndTsDocResolver, workspaceUris, documentManager },
    );
    ...
    this.signatureHelpProvider = new SignatureHelpProviderImpl(
      this.lsAndTsDocResolver,
    );
    this.semanticTokensProvider = new SemanticTokensProviderImpl(
      this.lsAndTsDocResolver,
    );
    this.implementationProvider = new ImplementationProviderImpl(
      this.lsAndTsDocResolver,
    );
    this.typeDefinitionProvider = new TypeDefinitionProviderImpl(
      this.lsAndTsDocResolver,
    );
  ...
  async getDiagnostics(
    ...
    console.log('🚀 ~ file: TypeScriptPlugin.ts:242 ~ document:', document);
    if (!this.featureEnabled('diagnostics')) {
      return [];
    }

    const result = this.diagnosticsProvider.getDiagnostics(
      document,
      cancellationToken,
    );
    result.then((value) => {
      console.log(
        '🚀 ~ file: TypeScriptPlugin.ts:250 ~ getDiagnostics:',
        value,
        { document },
      );
    });
    ...
    for (let symbol of symbols.slice(1)) {
      ...
      if (
        ...
        if (
          ...
          document
            .getText()
            .charAt(document.offsetAt(symbol.location.range.start)) !== 'p'
        ) {
  ...
  async getCodeActions(
    ...
    return this.codeActionsProvider.getCodeActions(
      document,
      range,
      context,
      cancellationToken,
  ...
  async resolveCodeAction(
    ...
    return this.codeActionsProvider.resolveCodeAction(
      document,
      codeAction,
      cancellationToken,
    );
  ...
  async onWatchFileChanges(
    ...
    for (const { fileName, changeType } of onWatchFileChangesParas) {
      ...
      const canSafelyIgnore =
        ...
        ignoredBuildDirectories.some((dir) => {
          ...
          return (
            ...
            index > 0 &&
            (dir !== '.svelte-kit' || dirPathParts[index + 1] !== 'types')
          );
        });
      if (canSafelyIgnore) {
        continue;
      ...
      const isClientSvelteFile =
        isSvelteFile &&
        this.documentManager.get(pathToUrl(fileName))?.openedByClient;
  ...
  async getImplementation(
    ...
    return this.implementationProvider.getImplementation(
      document,
      position,
      cancellationToken,
    );
  ...
  async getInlayHints(
    ...
    return this.inlayHintProvider.getInlayHints(
      document,
      range,
      cancellationToken,
    );
  ...
  resolveCodeLens(
    ...
    return this.codLensProvider.resolveCodeLens(
      document,
      codeLensToResolve,
      cancellationToken,
    );
