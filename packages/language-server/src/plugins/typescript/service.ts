...
export interface LanguageServiceDocumentContext {
  ...
  reportConfigError:
    | ((diagnostics: PublishDiagnosticsParams) => void)
    | undefined;
...
export async function getService(
  ...
  const nearestBoundary =
    (nearestNodeModulesBoundary?.length ?? 0) >
    (nearestWorkspaceUri?.length ?? 0)
...
async function forAllServicesWorker(
  cb: (service: LanguageServiceContainer) => any,
): Promise<void> {
...
export async function getServiceForTsconfig(
  ...
  if (reloading || !services.has(tsconfigPathOrWorkspacePath)) {
    if (reloading) {
      Logger.log(
        'Reloading ts service at ',
        tsconfigPath,
        ' due to config updated',
      );
    ...
    const newService = createLanguageService(
      tsconfigPath,
      workspacePath,
      docContext,
    );
...
async function createLanguageService(
  ...
  function createSnapshotManager(
    ...
    const cached = configFileName
      ? parsedTsConfigInfo.get(configFileName)
      : undefined;
  ...
  function updateSnapshot(
    ...
    console.log(
      '🚀 ~ file: typescript/service.ts:583 ~ updateSnapshot ~ documentOrFilePath:',
      documentOrFilePath,
    );
  ...
  function updateSnapshotFromDocument(document: Document): DocumentSnapshot {
    ...
    console.log(
      '🚀 ~ file: typescript/service.ts:596 ~ prevSnapshot:',
      prevSnapshot,
      { filePath, document },
    );
  ...
  function getSnapshotIfExists(fileName: string): DocumentSnapshot | undefined {
    ...
    let doc =
      snapshotManager.get(fileName) ?? snapshotManager.get(svelteFileName);
    if (doc) {
      return doc;
    }
  ...
  function getScriptFileNames() {
    ...
    const canonicalProjectFileNames = new Set(
      projectFiles.map(getCanonicalFileName),
    );
  ...
  function fileBelongsToProject(filePath: string, isNew: boolean): boolean {
    ...
    return (
      hasFile(filePath) ||
      (isNew && getParsedConfig().fileNames.includes(filePath))
    );
  }
  ...
  function watchConfigFiles(
    ...
    if (!configWatchers.has(tsconfigPath) && tsconfigPath) {
      configFileModifiedTime.set(
        tsconfigPath,
        tsSystem.getModifiedTime?.(tsconfigPath),
      );
      configWatchers.set(
        tsconfigPath,
    ...
    for (const config of tsconfigDependencies) {
      ...
      dependedConfigWatchers.set(
        ...
        tsSystem.watchFile(
          config,
          createWatchDependedConfigCallback(docContext),
          1000,
        ),
      );
    }
  ...
  function updateIfDirty() {
    ...
    if (!skipSvelteInputCheck) {
      ...
      projectConfig.errors.splice(
        0,
        projectConfig.errors.length,
        ...new_errors,
      );
    }
  ...
  function initLsCacheProject() {
    ...
    const createLanguageServiceForAutoImportProvider = (
  ...
  function ensureTsConfigInfoUpToDate(configFilePath: string) {
    ...
    const content =
      tsSystem.fileExists(configFilePath) && tsSystem.readFile(configFilePath);
    if (!content) {
  ...
  function ensureFilesForConfigUpdates(info: TsConfigInfo | null) {
    if (info?.pendingProjectFileUpdate) {
      ...
      info.parsedCommandLine.fileNames =
        info.snapshotManager.getProjectFileNames();
    }
  }
 ...
 */
function createWatchDependedConfigCallback(
  docContext: LanguageServiceDocumentContext,
) {
  return async (
    fileName: string,
    ...
    const promises = Array.from(
      configPathToDependedProject.get(fileName) ?? [],
    ).map(async (config) => {
