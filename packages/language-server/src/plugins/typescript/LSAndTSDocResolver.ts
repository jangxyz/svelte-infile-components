...
export class LSAndTSDocResolver {
  constructor(
    ...
    console.log(
      '🚀 ~ file: LSAndTSDocResolver.ts:59 ~ { docManager, workspaceUris, configManager, options }:',
      { docManager, workspaceUris, configManager, options },
    );
    ...
    this.packageJsonWatchers = new FileMap(
      this.tsSystem.useCaseSensitiveFileNames,
    );
    this.watchedDirectories = new FileSet(
      this.tsSystem.useCaseSensitiveFileNames,
    );
    ...
    this.lsDocumentContext = {
      ...
      ambientTypesSource: this.options?.isSvelteCheck
        ? 'svelte-check'
        : 'svelte2tsx',
  ...
  async getLSAndTSDoc(document: Document): Promise<{
    ...
    const { tsDoc, lsContainer, userPreferences } =
      await this.getLSAndTSDocWorker(document);
  ...
  async getLsForSyntheticOperations(document: Document): Promise<{
    ...
    return {
      tsDoc,
      userPreferences,
      lang: lsContainer.getService(/* skipSynchronize */ true),
    };
  }
  ...
  private async getLSAndTSDocWorker(document: Document) {
    ...
    console.log(
      '🚀 ~ file: LSAndTSDocResolver.ts:216 ~ getLSAndTSDocWorker ~ tsDoc:',
      tsDoc,
      {
        document,
        lsContainer,
  ...
  async getOrCreateSnapshot(
    document: Document,
  ): Promise<SvelteDocumentSnapshot>;
  async getOrCreateSnapshot(
    pathOrDoc: string | Document,
  ): Promise<DocumentSnapshot>;
  async getOrCreateSnapshot(pathOrDoc: string | Document) {
    const filePath =
      typeof pathOrDoc === 'string' ? pathOrDoc : pathOrDoc.getFilePath() || '';
  ...
  private async updateSnapshot(document: Document) {
    ...
    await this.updateExistingFile(filePath, (service) =>
      service.updateSnapshot(document),
    );
  }
  ...
  async updateProjectFiles(watcherNewFiles: string[]) {
    await forAllServices((service) =>
      service.scheduleProjectFileUpdate(watcherNewFiles),
    );
  }
  ...
  async getTSService(filePath?: string): Promise<LanguageServiceContainer> {
    //console.log('🚀 ~ file: LSAndTSDocResolver.ts:333 ~ filePath:', filePath, this.options?.tsconfigPath, { tsConfigPath: this.options?.tsconfigPath });
    ...
    if (!filePath) {
      throw new Error(
        'Cannot call getTSService without filePath and without tsconfigPath',
      );
    }
    ...
    return {
      ...
      readFile: (path, encoding) => {
        if (
          ...
          !this.packageJsonWatchers.has(path)
        ) {
  ...
  private onPackageJsonWatchChange(
    path: string,
    onWatchChange: ts.FileWatcherEventKind,
  ) {
