...
export class DiagnosticsProviderImpl implements DiagnosticsProvider {
  ...
  async getDiagnostics(
    ...
    console.log(
      '🚀 ~ file: typescript/DiagnosticsProvider.ts:96 ~ DiagnosticsProviderImpl ~ getDiagnostics ~ { lang, tsDoc }:',
      { lang, tsDoc, isTypescript },
    );
    ...
    console.log(
      '🚀 ~ file: typescript/DiagnosticsProvider.ts:117 ~ DiagnosticsProviderImpl ~ getDiagnostics ~ getSyntacticDiagnostics ~ diagnostics1:',
      diagnostics.length,
      diagnostics,
    );
    ...
    for (const checker of checkers) {
      ...
      const results = checker.call(lang, tsDoc.filePath);
      console.log(
        '🚀 ~ file: typescript/DiagnosticsProvider.ts:137 ~ lang, tsDoc.filePath:',
        checker,
        results.length,
        results,
      );
    ...
    console.log(
      '🚀 ~ file: typescript/DiagnosticsProvider.ts:139 ~ DiagnosticsProviderImpl ~ diagnostics2:',
      diagnostics,
      inspect(
        diagnostics.map((diag) => ({
          start: diag.start,
          length: diag.length,
          //  range1: { start: document.positionAt(diag.start || 0), end: document.positionAt((diag.start || 0) + (diag.length || 0)), },
          range2: convertRange(tsDoc, diag),
          position: {
            start: tsDoc.getOriginalPosition(convertRange(tsDoc, diag).start),
            end: tsDoc.getOriginalPosition(convertRange(tsDoc, diag).end),
          },
        })),
        { depth: 4 },
      ),
    ...
    for (const diagnostic of diagnostics) {
      if (
        (diagnostic.code === DiagnosticCode.NO_OVERLOAD_MATCHES_CALL ||
          diagnostic.code ===
            DiagnosticCode.ARG_TYPE_X_NOT_ASSIGNABLE_TO_TYPE_Y) &&
    ...
    console.log(
      '🚀 ~ file: typescript/DiagnosticsProvider.ts:200 ~ DiagnosticsProviderImpl ~ diagnostics3 (with store stuff):',
      diagnostics.length,
      diagnostics,
    );
    ...
    console.log(
      '🚀 ~ file: typescript/DiagnosticsProvider.ts:212 ~ DiagnosticsProviderImpl ~ diagnostics4 (expecting transition thrid arg):',
      diagnostics.length,
      diagnostics,
    );
    ...
    console.log(
      '🚀 ~ file: typescript/DiagnosticsProvider.ts:218 ~ DiagnosticsProviderImpl ~ diagnostics5 (resolve noops):',
      diagnostics,
    );
    ...
    console.log(
      '🚀 ~ file: DiagnosticsProvider.ts:235 ~ DiagnosticsProviderImpl ~ converting ts.Diagnostic to Diagnostic...',
    );
    ...
    for (const tsDiag of diagnostics) {
      ...
      console.log(
        '🚀 ~ file: typescript/DiagnosticsProvider.ts:236 ~ DiagnosticsProviderImpl ~ diagnostic1 (as Diagnostic):',
        structuredClone(diagnostic),
        { tsDiag },
      );
      // convert back to position based on .svelte source file
      diagnostic = mapRange(diagnostic);
      console.log(
        '🚀 ~ file: typescript/DiagnosticsProvider.ts:243 ~ DiagnosticsProviderImpl ~ diagnostic2 (map ranges):',
        structuredClone(diagnostic),
      );
      ...
      console.log(
        '🚀 ~ file: typescript/DiagnosticsProvider.ts:255 ~ DiagnosticsProviderImpl ~ diagnostic3:',
        diagnostic,
      );
      diagnostic = swapDiagRangeStartEndIfNecessary(diagnostic);
      console.log(
        '🚀 ~ file: typescript/DiagnosticsProvider.ts:260 ~ DiagnosticsProviderImpl ~ diagnostic4:',
        diagnostic,
      );
      converted.push(diagnostic);
    }
    console.log(
      '🚀 ~ file: typescript/DiagnosticsProvider.ts:266 ~ DiagnosticsProviderImpl ~ converted:',
      inspect(converted, { depth: 4 }),
    );
  ...
  private async getLSAndTSDoc(document: Document) {
    console.log(
      '🚀 ~ file: typescript/DiagnosticsProvider.ts:282 ~ document:',
      document,
    );
...
function moveBindingErrorMessage(
  ...
  if (
    ...
    tsDiag.start &&
    tsDoc
      .getText(tsDiag.start, tsDiag.start + tsDiag.length!)
      .endsWith('.$$bindings')
  ) {
...
/** Return a mapper function that maps position based on compiled ts file to svelte file. */
function rangeMapper(
  ...
  return (diagnostic: Diagnostic) => {
    ...
    console.log(
      '🚀 ~ file: typescript/DiagnosticsProvider.ts:348 ~ rangeMapper ~ mapping range to original ~ range:',
      range,
      { 'diagnostic.range': diagnostic.range, snapshot },
      '...',
      {
        originalStart: snapshot.getOriginalPosition(diagnostic.range.start),
        originalEnd: snapshot.getOriginalPosition(diagnostic.range.end),
      },
    );
    ...
    if (
      ...
      const node = getNodeIfIsInStartTag(
        document.html,
      ...
      if (node) {
        ...
        range.end = document.positionAt(
          node.start + 1 + (node.tag?.length || 1),
        );
      }
    }
...
function resolveNoopsInReactiveStatements(
  ...
  const isLet = (file: ts.SourceFile) => (node: ts.Node) => {
    ...
    return (
      !!defs &&
      defs.some((def) => def.fileName === file.fileName && def.kind === 'let')
    );
  };
  ...
  const expandedDiagnostics = flatten(
    passMap(diagnostics, expandRemainingNoopWarnings),
  );
...
function get$$PropsAliasForInfo(
  ...
  const type = lang
    .getProgram()
    ?.getTypeChecker()
    ?.getTypeAtLocation(propsDef.name);
  if (!type) {
    return;
  }
    ...
    if (propsStart) {
      return {
        start: propsStart,
          ...
          ...propsStart,
          character: propsStart.character + '$$Props'.length,
        },
      };
    }
