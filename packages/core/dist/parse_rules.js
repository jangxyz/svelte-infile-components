...
function extractExportNameTuples(node) {
    if (node.type === 'ExportNamedDeclaration') {
        if (node.declaration) {
            ...
            if (declaration.type === 'VariableDeclaration') {
                // we use filter because we can declare multiple variables
                const decls = declaration.declarations.filter(isVariableDeclaratorWithIdIdentifier);
                return decls.map((decl) => [decl.id.name, undefined, [node, decl]]);
            }
            // `export function getValue() { }`
            else if (declaration.type === 'FunctionDeclaration') {
                return [[declaration.id.name, undefined, [node, declaration]]];
            }
            // `export class C { }`
            else if (declaration.type === 'ClassDeclaration') {
                return [[declaration.id.name, undefined, [node, declaration]]];
    ...
    // `export default x = 3`
        ...
        if (decl.type === 'AssignmentExpression' && (0, parse_utils_js_1.isIdentifier)(decl.left)) {
            return [[decl.left.name, 'default', [node, decl]]];
        }
        else if (decl.type === 'Identifier') {
            return [[decl.name, 'default', [node, decl]]];
        }
        else if (decl.type === 'Literal') {
            return [[decl.raw, 'default', [node, decl]]];
            //} else if (decl.type === 'CallExpression') {
            //  return [[decl., 'default', [node, decl]] as const];
        }
        else {
...
//# sourceMappingURL=parse_rules.js.map