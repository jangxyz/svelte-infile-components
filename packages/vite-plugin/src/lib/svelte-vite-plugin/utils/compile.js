...
export function createCompileSvelte() {
  ...
  return async function compileSvelte(svelteRequest, code, options) {
    ...
    if (options.stats) {
      if (options.isBuild) {
        if (!stats) {
          ...
          stats = options.stats.startCollection(
            `${ssr ? 'ssr' : 'dom'} compile`,
            {
              logInProgress: () => false,
            },
    ...
    if (preprocessors) {
      try {
        preprocessed = await svelte.preprocess(code, preprocessors, {
          filename,
        }); // full filename here so postcss works
      } catch (e) {
      ...
      if (preprocessed.dependencies?.length) {
        const checked = checkPreprocessDependencies(
          filename,
    ...
    let compiled;
    try {
      compiled = svelte.compile(finalCode, {
        ...finalCompileOptions,
        filename,
      });
